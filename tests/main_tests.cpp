...
TEST_CASE("Main test for checking the correct execution of the algorithm") {
    ...
    for (int i = n; i > 0; i--) {
        ...
        fwrite(&RandomNumber, n_size, 1, s);
    }
    fclose(s);
    ...
    polyphase_merge_sort(s_file, 5, n, 10);
    auto end = steady_clock::now();


    FILE * oput = fopen("0.dat","rb");
    int ReadedNumber;
    while (fread(&ReadedNumber, n_size, 1, oput)) {
        cout << ReadedNumber << ", ";
    }
    cout << "\n\n\n";

    bool isCorrect = is_sorted();
